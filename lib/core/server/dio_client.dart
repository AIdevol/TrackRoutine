...
class AppDioLogger extends Interceptor {
  ...
  final void Function(Object object, {bool isError}) logPrint;

  static void _defaultPrint(Object object, {bool isError = false}) {
    log(object.toString());
  }
  ...
  AppDioLogger({
    ...
    this.maxWidth = 90,
    this.compact = true,
    void Function(Object obj, {bool isError})? logPrinter,
  }) : logPrint = logPrinter ?? _defaultPrint;
  ...
  void _printResponse(
    ...
    if (response.data != null) {
      if (response.data is Map) {
        ...
        _printUint8List(response.data as Uint8List, isError: isError);
        logPrint(
          '║${_indent()}]',
          isError: isError,
        );
        ...
        _printList(response.data as List, isError: isError);
        logPrint(
          '║${_indent()}]',
          isError: isError,
        );
  ...
  void _printLine({String pre = '', String suf = '╝', bool isError = false}) =>
      logPrint(
        '$pre${'═' * maxWidth}$suf',
        isError: isError,
      );
  ...
  void _printBlock(String msg, {bool isError = false}) {
  ...
  void _printPrettyMap(
    ...
    if (isRoot || isListItem) logPrint('║$initialIndent{', isError: isError);
    ...
    data.keys.toList().asMap().forEach((index, dynamic key) {
      ...
      if (value is Map) {
        if (compact && _canFlattenMap(value)) {
          ...
          logPrint('║${_indent(tabs)} $key: {', isError: isError);
        ...
        if (compact &&
          ...
          logPrint('║${_indent(tabs)} $key: $value', isError: isError);
        } else {
          logPrint('║${_indent(tabs)} $key: [', isError: isError);
          _printList(value, tabs: tabs);
          logPrint('║${_indent(tabs)} ]${isLast ? '' : ','}', isError: isError);
        }
      } else {
  ...
  void _printUint8List(
    ...
    for (final element in chunks) {
      logPrint('║${_indent(tabs)} ${element.join(", ")}', isError: isError);
    }
  }
